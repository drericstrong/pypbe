...
for system in ["pf", "3e", "4e"]:
    big_res = []
    for comb in inputs:
        pbe_gen = pbe.PBE(comb[0], comb[2], comb[3], comb[4], comb[6], comb[7],
                          comb[1], comb[5], pbe_map=system)
        pbe_gen.roll_mc(10**6)
        res = pbe_gen.get_results()
        res.append(comb)
        big_res.append(res)     
    df = pd.DataFrame(big_res)
    df.columns = ["Description", "Typical Array", "Mean", "Standard Deviation", "5%", "95%", "Input"]
